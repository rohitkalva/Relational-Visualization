  ...
  Initialize = () => {
        ...
        for (let i = 0; i < this.matrixIndex.length; i++) {
          ...
          for (let j = 0; j < this.matrixIndex.length; j++) {
            objs = this.dataStore.filter(function (obj) {
              return _this.matrix.filter(obj, _this.indexHash[_this.matrixIndex[i]], _this.indexHash[_this.matrixIndex[j]]);
            });
    ...
    read : (d) => {
      ...
      if (d.source) {       
        ...
        //m.kwVizState = d.source.value.data[0].keywordVisibility;
        //m.taxVizState = d.source.value.data[0].taxonomyVisibility;
        //m.tid  = d.source.value.data[0].taxId;
        let row = d.source.value.data.find(x => x.taxonomyName === d.target._id || x.keywordName === d.target._id);
        // let row = d.source.value.data.find(x => x.taxonomyName === d.target._id);
        if(row === undefined) {
          row = {
            taxonomyName: '',
            keywordName: '',
        ...
        m.tname  = row.taxonomyName;
        m.kname  = row.keywordName;
        m.tid  = row.taxId;
        m.tvalue = row.spectCount;
        m.kid  = row.keywordId;
